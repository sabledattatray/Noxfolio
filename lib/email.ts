...
export async function sendVerificationEmail(email: string, otp: string) {
  try {
    const { data, error } = await resend.emails.send({
      from: 'BillForge <onboarding@resend.dev>', // Replace with your verified domain in production
      to: [email],
      subject: 'Verify your BillForge account',
      html: `
        <div style="font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #f9fafb; padding: 40px 20px; text-align: center;">
          <div style="max-width: 480px; margin: 0 auto; background-color: #ffffff; border-radius: 24px; padding: 48px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06);">
            ...
            <p style="font-size: 16px; color: #4b5563; margin-bottom: 32px; line-height: 1.5;">Welcome to BillForge! Use the verification code below to complete your sign-up.</p>
          ...
          <p style="margin-top: 24px; font-size: 12px; color: #9ca3af;">&copy; 2024 BillForge Foundation. All rights reserved.</p>
        </div>
      `,
    });
